...
SELECT
	...
	, cc.DateRecorded
	, cc.DailyCases
	,sum(cc.DailyCases) over (partition by Country order by cc.DateRecorded) AS CumulativeCases
...
SELECT
	uk.DateRecorded
	, uk.DailyCases
	,sum(DailyCases) over (order by DateRecorded) AS CumulativeCases
...
SELECT 
	uk.DateRecorded
	, uk.DailyCases
	,row_number() over(partition by uk.DailyCases order by uk.DailyCases desc ) as rank
...
SELECT
row_number() over (partition by Country, DateRecorded order by totalcases desc ) as rank
, *
FROM
(
select 
Country, T.DateRecorded,
SUM(T.DailyCases) over (partition by Country, T.DateRecorded order by ) 
